...
var category_url =
	"https://openlibrary.org/subjects/mystery_and_detective_stories.json";
...
$.ajax({
	...
	.done(function (data) {
		...
		if (data.ebook_count == 0) {
			$(".row.books")
				.addClass("justify-content-center my-40")
		...
		$.each(data.works, function (index, bookInfo) {
			...
			<div class="book-cover"><img src="https://covers.openlibrary.org/b/id/${
				bookInfo.cover_id
			}-M.jpg"></div>
			<div class="book-hidden">
				<a href="https://openlibrary.org${
					bookInfo.key
				}" class="btn btn-info" target="_blank">More Info</a>
				${
					bookInfo.availability.is_readable
						? `<a href="https://openlibrary.org/borrow/ia/${bookInfo.ia}?ref=ol" class="btn btn-read" target="_blank">Read Book</a>`
						: ""
				}
			</div>
		</div>`;
	...
	.fail(function () {
		$(".loader").hide();
		$(".row.books")
			.addClass("justify-content-center error my-3")
			.html(
				'<h3 class="text-center">Loading of books failed...<br>An error has occurred.</h3>'
			);
	});

//Task1
$("#humburger").click(function () {
	if ($(this).hasClass("opend-humburger")) {
		$(this).empty();
		$(this).append("&equiv;");
	} else {
		$(this).empty();
	...
	$(this).toggleClass("opend-humburger");
...
$("#readers-plus").click(function () {
	let num = Number($(this).attr("data-num")) + 1;
	$("#num-of-readers").text(`You are ${num} reader now`);

	let a = $(this).closest(".row").find(".book > .book-hidden > .btn-read")[0];
...
$(".cat-link").click(function (e) {
	...
	$(".cat-link.active").toggleClass("active");
	$(this).toggleClass("active");

	let href = $(this).attr("href");

	getBooksByHref(href + ".json");
});

function getBooksByHref(href) {
	$.ajax({
		url: href,
		dataType: "json",
		beforeSend: function () {
